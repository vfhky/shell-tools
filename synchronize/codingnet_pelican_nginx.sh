#!/bin/bash
# FileName:		 codingnet_pelican_nginx.sh
...
# Simple Usage:	 ./codingnet_pelican_nginx.sh "commit_comments"
# Crontab Usage: 00 01 * * * /mydata/backups/bak_list/codingnet_pelican_nginx.sh >/dev/null 2>&1
# (c) 2016 vfhky https://typecodes.com/linux/syngithubmarkdownpelican.html
# https://CodingNet/vfhky/shell-tools/blob/master/synchronize/codingnet_pelican_nginx.sh
...
CODINGNET_PELICAN_DIR=/mydata/GitBang/CodingNet/BlogBak
...
# Check parameter.
if [ $# -gt 1 ]; then
	echo "Usage:	./codingnet_pelican_nginx.sh 1 \"commit_comments\"" && exit 1
fi
...
# Main process begin.
NOTICE "[1]Start pull from CodingNet."
RC=0
RUNCMD "cd ${CODINGNET_PELICAN_DIR}/md_article && Git_Pull"
...
NOTICE "[2.1]Start copy the pulled articles to the compile dir of PELICAN."
# New_Article_Files=$(Get_Files_Path ${CODINGNET_PELICAN_DIR}/md_article)
...
${FINDCMD} . -mmin -${TIME_GAP} -type f -name "*.md"|while read -r New_Article_File
do
	if [ -z "${New_Article_File}" ] && [ "$1"x != "1"x ]; then
...
NOTICE "[2.2]Start copy the pulled program to the compile dir of PELICAN."
# New_Article_Files=$(Get_Files_Path ${CODINGNET_PELICAN_DIR}/program/pelican/pelican-vfhky)
# You should not delete the double quotation marks in case of existing a blank in the file path.
#### Method 1: for New_Article_File in `${FINDCMD} . -mmin -${TIME_GAP} -type f -name "*.md"`
#### Method 2: use the while recycle.
RUNCMD "cd ${CODINGNET_PELICAN_DIR}/program/pelican/"
${FINDCMD} . -mmin -${TIME_GAP} -type f|while read -r New_Program_File
do
	if [ -z "${New_Program_File}" ]; then
		echo "No new program file."
		break
	fi
	PROGRAM_PATH=$(dirname ${PELICAN_COMPILE_DIR}/"${New_Program_File:2}")
	RUNCMD "${CPCMD} \"${New_Program_File}\" ${PROGRAM_PATH}"
done
...
RC=$?
if [ $RC -gt 0 ]; then
	ERROR "Copy the pulled program failed!"
fi
